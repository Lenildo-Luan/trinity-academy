...
export async function generateMetadata({
  ...
  let lesson = await getLesson("pre-calculo", (await params).slug);

  return {
    title: `${lesson?.title} - Pré-Cálculo`,
...
export default async function Page({
  ...
  const moduleName = "pre-calculo";
  let slug = (await params).slug;
  let lesson = await getLesson("pre-calculo", slug);
  ...
  let quiz = lesson.quizId
    ? await getQuiz("pre-calculo", lesson.quizId)
    : null;
  ...
  return (
    <SidebarLayoutContent
      breadcrumbs={
        <Breadcrumbs>
          <BreadcrumbHome href={"/pre-calculo"}>Pré-Cálculo</BreadcrumbHome>
