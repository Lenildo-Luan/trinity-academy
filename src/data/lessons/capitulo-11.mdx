# Aula 9: Arrays - Trabalhando com Listas de Dados

## Introdução: O Poder das Listas em Programação
...
### 1. Arrays vs Outras Estruturas
- Arrays são ideais para **sequências ordenadas**
- Objetos são melhores para **propriedades nomeadas**
- Cada estrutura é otimizada para diferentes padrões de acesso

### 2. Indexação Base Zero
- É uma **convenção estabelecida** na tecnologia
- Primeiro elemento: índice 0
- Último elemento: índice `length - 1`

### 3. Performance dos Métodos
- `push/pop` são **rápidos** (trabalham no final)
- `shift/unshift` são **mais lentos** (precisam renumerar índices)

### 4. Formas de Iteração
- **for tradicional**: máximo controle, acesso ao índice
- **for...of**: código limpo, foco no valor
- **forEach**: estilo funcional, não permite break

### 5. Mutabilidade
- Métodos como `push`, `pop`, `shift`, `unshift` **modificam** o array original
- Sempre verifique se o método muda o array ou retorna um novo
...
- [ ] Explicar o que são arrays e quando usá-los
- [ ] Criar arrays de diferentes formas
- [ ] Entender por que a indexação começa em zero
- [ ] Acessar elementos através de índices
- [ ] Usar a propriedade `length`
- [ ] Modificar elementos existentes
- [ ] Adicionar elementos com `push()` e `unshift()`
- [ ] Remover elementos com `pop()` e `shift()`
- [ ] Percorrer arrays com for, for...of e forEach
- [ ] Explicar a diferença de performance entre métodos
- [ ] Compreender mutabilidade de arrays

---

## 🚀 Próximos Passos

Na próxima aula (Dia 10), exploraremos **métodos avançados de arrays** como:
...
Esses métodos nos darão superpoderes para processar e analisar dados de forma eficiente!

---

## 📝 Resumo da Aula

**Arrays** são estruturas fundamentais para armazenar coleções ordenadas de dados. Aprendemos que:

1. Arrays usam **indexação baseada em zero**
2. A propriedade **`length`** nos dá o tamanho do array
3. Podemos adicionar/remover no **final** (push/pop) ou **início** (unshift/shift)
4. Existem **três formas principais** de percorrer arrays
5. Métodos que trabalham no final são **mais eficientes**
6. Arrays são **mutáveis** - cuidado ao modificá-los!

Com este conhecimento fundamental, você está preparado para construir aplicações que gerenciam listas de dados de forma eficiente e profissional. Continue praticando e nos vemos na próxima aula! 🎯
