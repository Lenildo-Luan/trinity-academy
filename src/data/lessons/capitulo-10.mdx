...
## 1. Arrow Functions: Uma Sintaxe Mais Elegante
...
## 2. Funções Como Valores: Cidadãos de Primeira Classe
...
## 3. Callbacks: Funções Que Esperam Sua Vez
...
## 4. Higher-Order Functions: Funções que Trabalham com Funções
...
## 5. Closures: Memória das Funções
...
## 6. Diferenças Entre Tipos de Função
...
### Exercício 1: Conversor de Temperaturas

Crie uma função que retorna conversores de temperatura:

```javascript
function criarConversor(de, para) {
    // Implemente aqui
    // Deve retornar uma função que converte temperatura
}

const celsiusParaFahrenheit = criarConversor("C", "F");
const fahrenheitParaCelsius = criarConversor("F", "C");

console.log(celsiusParaFahrenheit(0));   // 32
console.log(celsiusParaFahrenheit(100)); // 212
console.log(fahrenheitParaCelsius(32));  // 0
```

### Exercício 2: Sistema de Notas

Crie um sistema que processa notas de alunos:
...
// Use arrow functions e callbacks para:
// 1. Filtrar alunos aprovados (nota >= 7)
// 2. Mapear para mostrar nome e situação
...
```javascript
function criarProcessadorTexto() {
    return {
        maiusculas: // função arrow aqui,
        minusculas: // função arrow aqui,
        capitalizar: // função arrow aqui,
        aplicar: // função que recebe um callback
    };
}
```
...
1. **Arrow Functions** são uma sintaxe mais concisa para escrever funções
2. **Funções são valores** em JavaScript (cidadãos de primeira classe)
3. **Callbacks** são funções passadas como argumentos para serem executadas depois
4. **Higher-order functions** trabalham com outras funções (recebem ou retornam)
5. **Closures** permitem que funções "lembrem" de variáveis do seu escopo de criação
6. **Escolha o tipo de função** baseado no contexto e na clareza do código

---

## Resumo

Nesta aula, você aprendeu sobre conceitos avançados de funções que tornam JavaScript uma linguagem extremamente flexível e poderosa:

- **Arrow functions** oferecem sintaxe concisa e clara
- **Callbacks** permitem passar comportamentos como dados
- **Higher-order functions** criam abstrações poderosas
- **Closures** preservam contexto e criam encapsulamento
- **Diferentes tipos de função** servem diferentes propósitos

Esses conceitos são fundamentais para programação moderna em JavaScript e abrem portas para padrões avançados de programação funcional que você usará em frameworks e bibliotecas populares.
