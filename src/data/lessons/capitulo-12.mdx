# Métodos Avançados de Arrays em JavaScript
...
## 1. Buscando Elementos: includes(), indexOf() e find()
...
## 2. Filtrando Dados: O Método filter()
...
## 3. Transformando Dados: O Método map()
...
## 6. Combinando Métodos (Encadeamento)
...
## 9. Conceitos Importantes para Fixar
...
let livros = [
    ...
    {titulo: "Harry Potter", autor: "J.K. Rowling", ano: 1997, disponivel: true},
    {titulo: "Dom Casmurro", autor: "Machado de Assis", ano: 1899, disponivel: true}
];

// Desafios:
// 1. Liste apenas livros disponíveis
// 2. Encontre um livro específico pelo título
// 3. Crie lista com apenas os títulos
// 4. Ordene por ano de publicação
// 5. Verifique se todos os livros foram publicados depois de 1800
```

### Exercício 2: Análise de E-commerce
...
// Desafios:
// 1. Calcule o subtotal de cada item (preco * quantidade)
// 2. Encontre o item mais caro
// 3. Verifique se algum item custa mais de R$ 1000
// 4. Liste os nomes dos produtos em ordem alfabética
```
...
Os métodos avançados de arrays são ferramentas poderosas que tornam seu código:
- **Mais legível**: Expressam intenções claramente
- **Mais conciso**: Menos linhas de código
- **Mais seguro**: Não modificam dados originais (exceto sort)
- **Mais profissional**: Seguem padrões modernos de JavaScript

**Próximos Passos:**
1. Pratique cada método individualmente
2. Experimente combinar métodos (encadeamento)
...
✅ **includes()**: Verifica existência (retorna boolean)  
✅ **indexOf()**: Encontra posição (retorna índice ou -1)  
✅ **find()**: Busca por condição (retorna elemento ou undefined)  
✅ **filter()**: Separa elementos por condição (retorna novo array)  
✅ **map()**: Transforma todos os elementos (retorna novo array)  
✅ **some()**: Verifica se ALGUM atende (retorna boolean)  
✅ **every()**: Verifica se TODOS atendem (retorna boolean)  
✅ **sort()**: Ordena elementos (modifica original!)  
✅ **Encadeamento**: Combine métodos para operações complexas

Agora você está pronto para manipular arrays como um profissional! 🚀
